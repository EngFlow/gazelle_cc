...
func runTestCase(t *testing.T, readOnlyTestDir, repositoryDir string) {
	...
	t.Logf("==> [%s] Running conan install...", testDir)
	conanConfig := tests.ExecConfig{Dir: testDir, Env: append(os.Environ(),
		"CONAN_HOME="+t.TempDir(), // unfortunate requirement, no write access to ~/.conan2
		"CONAN_NON_INTERACTIVE=1",
		"CONAN_SKIP_STATS=1"),
	}
	// Can be replaced by --install arg to conan indexer
	tests.Execute(conanConfig, t, "conan", "profile", "detect")
	tests.Execute(conanConfig, t, "conan", "install", ".", "--build=missing")
	...
	tests.Execute(bazelConfig, t, "bazel", "--output_base="+bazelOutputBase,
		"run", "@gazelle_cc//index/conan",
		"--", "--verbose", "--output="+indexPath, testDir)
