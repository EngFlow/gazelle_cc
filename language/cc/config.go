...
func (c *ccLanguage) Configure(config *config.Config, rel string, f *rule.File) {
	...
	for _, d := range f.Directives {
		...
		case cc_generate_proto:
			selectDirectiveChoice(&conf.generateProto, generateProtoModes, d)
...
type ccConfig struct {
	...
	generateCC bool
	// How `cc_proto_library` and `cc_grpc_library` rules be generated
	generateProto generateProtoMode
...
func newCcConfig() *ccConfig {
	return &ccConfig{
		...
		generateCC:              true,
		generateProto:           generateProtoMode_default,
		platforms:               map[platform.Platform]platformConfig{},
	}
}
...
type generateProtoMode string

var generateProtoModes = []generateProtoMode{generateProtoMode_disable, generateProtoMode_default, generateProtoMode_composedGrpc}

const (
	// Don't generate cc_proto_library or cc_grpc_library rules
	generateProtoMode_disable generateProtoMode = "disable"
	// Expect proto_library rules to be generated by "proto" language extension,
	// and generate cc_proto_library and cc_grpc_library rules based on them
	generateProtoMode_default generateProtoMode = "default"
	// Use cc_grpc_library rules for everything, thanks to grpc_only=False
	// attribute. The "proto" language extension should be disabled to avoid
	// rule conflicts.
	generateProtoMode_composedGrpc generateProtoMode = "composed_grpc"
)
