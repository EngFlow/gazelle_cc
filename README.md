...
1. **cc_library**: Created for:
   ...
   - Pregenerated `.pb.h` files in case when generation if `cc_proto_library` rules is disabled `# gazele:proto [legacy|disable|disable_global]`
...
4. **cc_proto_library**: Created for:
   ...
   - Generated only if `cc_proto_library` rules enabled generation of rules, that is `# gazele:proto [default|file|package]`
...
The `cc_binary` rule is always generated once per found translation unit containing `main` method

## Dependency Resolution

Dependency resolution between both internal and external dependencies is based only on `#include` directives used in sources. Gazelle C++ extension does parse the C/C++ source files to extract required information used preprocessor directives.

### Internal dependencies

Every build target managed by Gazelle C++ extension does register information about the header files defined in `hdrs` attribute of each `cc_library` rule. It allows to create an index of fully-qualified paths relative to the root directory of the repository.

Each source file path excluded from `#include` directives is looked up in the index, if a target rule can be found it would be added to the list of rule dependencies.
...
Rules/subdirectories that are not managed by the Gazelle are not populating the internal dependencies index and would not be automatically resolved. Gazelle can be instructed to use a user defined resolution rules to workaround this limitation
...
Other package managers [Conan](https://docs.conan.io/2/integrations/bazel.html), [vcpkg](https://vcpkg.io/en/) or rules for definig exteranl dependencies like [rules_foreign_cc](https://github.com/bazel-contrib/rules_foreign_cc) are currently not yet supported.

These still can be used by defining a manual mapping between header and defining rules using `# gazelle:resolve` directives

## C++20 Modules support

C++20 modules are currently not yet supported, but are planned to be introduced in the future.

## Example Usage

Here's an example of how to use the extension in your C++ project:

1. Provide default configuration in `BUILD.bazel` created in [installation step](#installation) and provide configuration for gazelle (optional):

```python
## Exclude following subtrees from being managed by gazelle. Be aware that it also prevents other targets from automatic dependency resolution to these modules using gazelle
# gazelle:exclude third-party
# gazelle:exclude examples/usage

## Define the cc sources should be managed, use `unit` for small file-based targets. 
# gazelle:cc_group unit

## Warn if there some unresolved cyclic dependencies between source files
