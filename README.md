...
### `# gazelle:cc_generate_proto [disable|default|composed_grpc]`

Controls how `cc_proto_library` and `cc_grpc_library` rules are generated:

- `disable`: Don't generate `cc_proto_library` or `cc_grpc_library` rules
- `default`: Expect `proto_library` rules to be generated by *"proto"* language extension, and generate `cc_proto_library` and `cc_grpc_library` rules based on them **(default)**
- `composed_grpc`: Use `cc_grpc_library` rules for everything, thanks to `grpc_only=False` attribute. The *"proto"* language extension should be disabled to avoid rule conflicts
